...
function TagChip({ tag }: { tag: string }) {
  ...
  const color = !valid
    ...
    : 'border-emerald-300 dark:border-emerald-700/50 bg-[var(--color-bg)] text-[var(--color-text-secondary)]';
    ...
    <div className="px-4 pb-4 pt-2 border-t border-[var(--color-border)] space-y-4">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          {listing.tags.length > 0 && (
            <button
              ...
              className="flex items-center gap-1 text-[11px] text-[var(--color-text-tertiary)] hover:text-[var(--color-text-secondary)] cursor-pointer border-none bg-transparent"
            >
        ...
        {listing.tags.length > 0 ? (
          ...
          <p className="text-sm text-[var(--color-text-tertiary)] italic">No tags</p>
        )}
        {(tagGaps > 0 || shortTags.length > 0 || singleWordTags.length > 0) && (
          <div className="mt-2 flex flex-wrap gap-2">
            ...
            {singleWordTags.length > 0 && (
              <span className="text-[11px] text-[var(--color-text-tertiary)] font-medium">{singleWordTags.length} single-word tag{singleWordTags.length > 1 ? 's' : ''}</span>
            )}
          </div>
        )}
      </div>
      ...
      <div>
        ...
        {listing.aiStatus === 'error' && (
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onRetryAI}
              className="text-xs text-[var(--color-brand)] hover:underline cursor-pointer border-none bg-transparent"
            >
              Retry
            </button>
          </div>
        ...
        {listing.aiStatus === 'pending' && (
          <p className="text-xs text-[var(--color-text-tertiary)] italic">Waiting for API key…</p>
        )}
      </div>

      {/* Description + Images + Price row */}
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <div className="bg-[var(--color-bg-subtle)] rounded-[var(--radius-lg)] p-3">
          <p className="text-[10px] text-[var(--color-text-tertiary)] uppercase tracking-wider font-semibold mb-1">Description</p>
          <p className="text-sm font-semibold text-[var(--color-text-secondary)]">{listing.description.length} chars</p>
          <p className="text-[11px] text-[var(--color-text-tertiary)] mt-0.5">
            {listing.description.length < 200 ? 'Too short — aim for 200+' : 'Good length'}
          </p>
        </div>
        <div className="bg-[var(--color-bg-subtle)] rounded-[var(--radius-lg)] p-3">
          <p className="text-[10px] text-[var(--color-text-tertiary)] uppercase tracking-wider font-semibold mb-1">Images</p>
          <p className="text-sm font-semibold text-[var(--color-text-secondary)]">{listing.images.length}/10 filled</p>
          <p className="text-[11px] text-[var(--color-text-tertiary)] mt-0.5">
            {listing.images.length < 10 ? `${10 - listing.images.length} slot${10 - listing.images.length > 1 ? 's' : ''} unused` : 'Fully filled'}
          </p>
        </div>
        <div className="bg-[var(--color-bg-subtle)] rounded-[var(--radius-lg)] p-3">
          <p className="text-[10px] text-[var(--color-text-tertiary)] uppercase tracking-wider font-semibold mb-1">Price</p>
          <p className="text-sm font-semibold text-[var(--color-text-secondary)]">${listing.price.toFixed(2)}</p>
          <p className="text-[11px] text-[var(--color-text-tertiary)] mt-0.5">
